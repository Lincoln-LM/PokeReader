...
pub fn run_view(
    ...
    if screen.get_is_top_screen() {
        ...
        screen.paint_square(&black, x, y, 200, 160)?;

        x += 10;
        y += 4;
        let types = ["B","G","S","X","C","#"];
        let patches = rng.get_patches();
        for row in 0..9 {
            let mut row_string = "".to_owned();
            for column in 0..9 {
                row_string = format!("{} {}", row_string, types[patches[column][row] as usize]).to_owned();
            }
            screen.draw_string(&white, &row_string, x, y)?;
            y += 12;
        }
